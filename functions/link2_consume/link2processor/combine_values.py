...
class CombinedValuesProcessor(object):
        ...
        if combined_fields:
            ...
            if xml_field_config:
                ...
                # If combination method is newline
                    ...
                    logging.error(f"Combination method {combination_method} defined in config is unknown")
                    return False, field_value
            else:
                logging.error(f"The 'combined_json_fields' or 'combined_xml_fields' field does not contain field {field}")
                return False, field_value
    ...
    def combined_value_xml(self, field, combined_fields, input_json):
        ...
        if not to_combine_info:
            logging.error(f"Could not find field {field} in field 'combined_xml_fields' in config")
            return False, ""
            ...
            if len(field_values) == 1:
                ...
                for row in field_values:
                    ...
                    for field in row:
                        ...
                        if combined_value:
                            if to_combine_info["start_with_field"]:
                                combined_value = combined_value + f"-{field}:{com_json_value}"
                            else:
                                combined_value = combined_value + f"-{com_json_value}"
                        else:
                            # If the combination should start with the original JSON field
